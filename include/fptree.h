...
class Btree : public Index<K, V, size>
{
	...
	{
		// TODO log
		leaf_node_t *newleaf = new (pmalloc(sizeof(leaf_node_t))) leaf_node_t;
	...
	static inline unsigned char hashfunc(K val)
	{
		...
		int i;
		uint64_t tempval = (uint64_t)val;
		for (i = 0; i < sizeof(K); i++)
		{
			uint64_t octet = tempval & 0x00ff;
			tempval = tempval >> 8;
  ...
  public:
	Btree()
	{
		...
		anchor = new (pmalloc(sizeof(leaf_node_t))) leaf_node_t;
		root = anchor;
		printf("***** New FP tree **** \n");
	}
	...
	bool modify(K key, V value, int modify_type)
	{
	...
	{
		return modify(key, value, 0);
	}

	bool update(K key, V value)
	{
		return modify(key, value, 1);
	}

	bool remove(K key)
	{
		return modify(key, V(-1), 2);
	}
